...
for index in tqdm(os.listdir(path_ds)):
    ...
    # Copy image
    shutil.copy(os.path.join(path_ds, index, "images", index + ".png"),
                os.path.join(path_sample, "imaging.png"))
    # result list
    seg_res = []
    # Combine segmentation masks
    for mask_file in os.listdir(os.path.join(path_ds, index, "masks")):
        seg_raw = Image.open(os.path.join(path_ds, index, "masks", mask_file))
        # Prepare mask
